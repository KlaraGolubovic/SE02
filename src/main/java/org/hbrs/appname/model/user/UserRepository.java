...
public interface UserRepository extends JpaRepository<User, Integer> {
    @Query("select new org.hbrs.appname.model.user.dto.UserDTOImpl(user) from User user where user.occupation=:occupation")
    List<UserDTO> findUsersByOccupation(@Param("occupation") String occupation);

    @Query("select new org.hbrs.appname.model.user.dto.UserDTOImpl(user) from User user where user.userid=:userId and user.password=:password")
    UserDTO findUserByIdAndPassword(@Param("userId") String id, @Param("password") String password);

    @Query("select new org.hbrs.appname.model.user.dto.UserDTOImpl(user) from User user where user.userid=:userId")
